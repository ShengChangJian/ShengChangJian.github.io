...
|项目|命名规范|
|文件名|全小写，单词用下划线隔开，加对应的后缀名|
}typename|大驼峰，不用加前缀|
|命名空间|大驼峰，加```N```前缀，建议具有真实含义的命名空间放在项目命名空间内|
|类名|大驼峰，加```C```前缀，形容词名词组合|
|枚举类型|大驼峰，加以```E```前缀|
|联合体类型|大驼峰，加以```U```前缀|
|C 结构|大驼峰，加以```S```前缀|
|类模板|大驼峰，加```T```前缀|
|接口|大驼峰，加```I```前缀|
|typedef 类型|后面加```_t```标识（虽然有点别扭）|
}函数名|大驼峰，动宾短语|
...
|宏定义|全大写，用下划线```_```隔开|
|常量(const)或枚举值|小驼峰，加以```k```前缀|

    注意：

名称中尽量不要使用单词缩写，除非是熟知的专业术语（如 URL、URI）。但同时在正确表义的情况下减少
...
编译器对源代码的排版没有要求，但好的排版对编写代码的人差错、修改和思维有很大的好处，同时也可增加
可读性。
...
+ 函数或语句一行写不下时：折行，并且元素要对齐，每行的末尾必须是符号（如```,```、```&&```等）；
+ 函数声明尽量要在一行，不能同行的，至少返回类型和函数名及```(```同行。
